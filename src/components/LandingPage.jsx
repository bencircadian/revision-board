...
export default function LandingPage({ onLoginSuccess }) {
  ...
  // 1. Social Login Handler (The "SSO" part)
  const handleSocialLogin = async (provider) => {
    setLoading(true);
    const { error } = await supabase.auth.signInWithOAuth({
      provider: provider,
      options: {
        redirectTo: window.location.origin, // Redirects back here after login
      },
    });
...
const landingStyles = `
  ...
  .input-group input:focus { border-color: #0d9488; outline: none; }
